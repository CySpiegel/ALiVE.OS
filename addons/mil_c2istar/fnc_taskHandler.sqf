...
switch (_operation) do {
    ...
    case "autoGenerateTasks": {
        if (_args isEqualType []) then {
            ...
            {
                private _taskID = _x;
                if (_taskID in keys _activeTasks) then {
                    _countActive = _countActive + 1;
                };
            } forEach (keys _sideTasks);
        ...
        if (_args isEqualType []) then {
            ...
            if (_taskCurrent == "Y") then {
                ...
                private _activeTasksToRemove = [];
                {
                    ...
                    if (_activeTaskID in keys _sideTasks) then {
...

            {
                ...
                if (_player in keys _tasksByPlayer) then {
            ...
            if (_taskApplyType == "Group") then {
...

                {
                    ...
                    if !(isNull _player) then {
                        ...
                        if (_group in keys _tasksByGroup) then {
                    ...
                    if (_taskApplyType == "Group") then {
                        ...
                        private _tasksByGroup = [_logic, "tasksByGroup"] call ALIVE_fnc_hashGet;
                        {
                            if !(_x in _updatedGroups) then {
                                if (_x in keys _tasksByGroup) then {
                                    // remove task
        ...
        private _handle = [_logic, _debug] spawn {
            ...
            waituntil {
...

                {
                    ...
                    if !(_managedTaskID in keys _activeTasks) then {
                        _managedTasksToRemove pushback _x;
                    };
                } forEach (keys _managedTasks);
                ...
                if (count (_managedTasks select 1) > 0) then {
                    ...
                    } forEach (keys _autoGenerateSides);
